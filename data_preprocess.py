...
class TweetsPreprocessor:
    def __init__(self, tweets_data_file=filename, year=2013):
        self.tweets_data_file = tweets_data_file
        self.output_dir = f'preprocessed_tweets_{year}'
        self.filenames = {
    ...
    def save_all_tweets(self):
        ...
        print(f"Saved all split tweets to {self.output_dir}!")
    
    def load_all_tweets(self, debug=False):
        if debug and os.path.exists(self.filenames['foreign_tweets']):
            for key, filename in self.filenames.items():
    ...
    def split_result_stats(self):
        ...
        print(f"---------- Splitting results finished ----------\n")
    ...
    def check_timestamp(self, tweet: dict):
        # check if timestamp in tweet is int; if not, change to int
        if "timestamp_ms" in tweet:
            if not isinstance(tweet["timestamp_ms"], int):
                tweet["timestamp_ms"] = int(tweet["timestamp_ms"])
    
    def preprocess_tweet(self, tweet: dict):
        self.check_timestamp(tweet)
    ...
    def preprocess_tweets(self):
        ...
        for i, tweet in enumerate(self.tweets):
            if i % 10000 == 0:
                print(f"Preprocessing tweet {i}, elapsed time: {time.time() - start_time}")
        ...
        print(f"---------- Preprocessing tweets finished ----------\n")
    ...
    def remove_golden_globes(self, text: str) -> str:
        ...
        text = re.sub(r'\bgolden globe(|s)\b', '', text, flags=re.IGNORECASE).strip()
        return re.sub(r'\bgoldenglobe(|s)\b', '', text, flags=re.IGNORECASE).strip()

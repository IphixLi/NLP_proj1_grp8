...
def nominee_extract(tweet: dict, base_confidence=1.0):
    if match_nominee_verb_pattern(tweet['new_text']):
        nominee_verb_based_match(tweet, base_confidence)
    if match_nominee_ts_pattern(tweet['new_text']):
        nominee_ts_based_match(tweet, base_confidence)
    nominee_winner_based_match(tweet, base_confidence)
...
def match_nominee_ts_pattern(text: str) -> bool:
    patterns = [
...
# def match_nominee_winner_pattern(text: str) -> bool:
#     patterns = [
#         r'\bor\b',
#         r'\bvs.\b',
#         r'\bbut\b',
...
def nominee_winner_based_match(tweet: dict, base_confidence=1.0):
    winner_match_list = winner_matcher.get_winner_from_text(tweet['new_text'])
    ...
    for winner_match in winner_match_list:
        ...
        award = winner_to_award[winner]
        is_human = about_human(award)
        
        award_keyword_in_tweet = []
        for token in spacy_output:
            if token.text.lower() in award_keywords and not token.text.lower() in award_keyword_in_tweet:
    ...
    # winner_list = winner_match['winner']
    # base_confidence *=  winner_match['confidence']
    # spacy_output = tweet.get('spacy_output', spacy_model(tweet['new_text']))
    # persons = find_persons(spacy_output)
    # works = find_work_of_art(spacy_output)
    
    # # check if person's names around the pattern
    # is_person_valid = False
    # if len(persons) > 1:
    #     for pattern_idx, match in all_matches:
    #         start, end = match.span()
    #         pre_names = [name for name in persons if tweet["new_text"].find(name.text) < start]
    #         post_names = [name for name in persons if tweet["new_text"].find(name.text) > end]
    #         if pre_names and post_names:
    #             is_person_valid = True
    #             if pattern_idx in worse_pattern_idx_list:
    #                 base_confidence *= 0.5
    #             break
    
    # # check if work of art's names around the pattern
    # is_work_valid = False
    # if len(works) > 1:
    #     for pattern_idx, match in all_matches:
    #         start, end = match.span()
    ...
    # if not is_person_valid and not is_work_valid:
    #     return
    ...
    #     is_human = about_human(award)
    #     if is_human:
    #         if not is_person_valid:
    #             continue
    #         winner_job = get_job(award)
    #         for p in persons:
    #             name = name_cleaning(p.text)
    #             space_count = name.count(" ")
    #             if name.lower() == winner:
    #                 continue
    #             # print(f"finding a name that is not winner: {name}")
    #             if space_count == 0 or space_count > 2:
    #                 candidates = {
    #                     "nominee_candidates": [name],
    #                     "award_candidates": [award],
    #                     "base_confidence": cur_confidence / (6 + 2 * space_count),
    #                 }
    #                 tweet["candidates"] = tweet.get("candidates", []) + [candidates]
    #             else:
    #                 candidates = {
    ...
    #                 }
    #                 tweet["candidates"] = tweet.get("candidates", []) + [candidates]
    #                 # job_list = wiki.possible_job_list(name)
    #                 # if job_list:
    #                 #     if winner_job in job_list:
    #                 #         candidates = {
    #                 #             "nominee_candidates": [p.text.lower()],
    #                 #             "award_candidates": [award],
    #                 #             "base_confidence": cur_confidence / len(job_list),
    #                 #         }
    #                 #         tweet["candidates"] = tweet.get("candidates", []) + [candidates]
    #                 # else:
    #                 #     candidates = {
    #                 #         "nominee_candidates": [name],
    #                 #         "award_candidates": [award],
    #                 #         "base_confidence": cur_confidence / 3,
    #                 #     }
    #                 #     tweet["candidates"] = tweet.get("candidates", []) + [candidates]                
    #     else:
    ...
    #             # TODO: check the category of the work
    #             candidates = {
    #                 "nominee_candidates": [w.text.lower()],
    #                 "award_candidates": [award],
    #                 "base_confidence": cur_confidence / 3,
    #             }
    #             tweet["candidates"] = tweet.get("candidates", []) + [candidates]
...
def nominee_ts_based_match(tweet: dict, base_confidence=1.0):
    spacy_output = tweet.get('spacy_output', spacy_model(tweet['new_text']))
    ...
    for ent in spacy_output.ents:
        satisfied = False
        if (ent.root.dep_ == "nsubjpass" or ent.root.dep_ == "nsubj") and ent.root.head.lemma_ in nominee_ts_verbs:
            satisfied = True
        else:
            for i in range(max(ent.start - 3, 0), min(len(spacy_output), ent.end + 6)):
                if spacy_output[i].lemma_ in nominee_ts_verbs:
                    satisfied = True
                    break
        if not satisfied:
            continue
    ...
    target_verb = [token for token in find_verbs(spacy_output) if token.lemma_ in nominee_ts_verbs]
    ...
    for possible_n, label in possible_nominees:
        ...
        if label == 'PERSON':
            ...
            if space_count > 0 and space_count <= 2:
                possible_roles = wiki.possible_job_list(name)
            # else:
            if space_count == 0 or space_count > 2:
        ...
        award, confidence = timestamp_cluster.categorize_timestamp(tweet['timestamp_ms'], possible_awards)

...
class Winner:
    def __init__(self, tweets: list, awards: list, base_confidence=1.0, year=2013):
        self.folder = f"winner_result_{year}"
    ...
    def extract_or_load(self, debug=False):
        print(f"---------- Extracting tweets for winners ----------")
    ...
    def do_vote(self, debug=False):
        print(f"---------- Voting for winners ----------")
        if debug and os.path.exists(f"{self.folder}/vote_{self.filename}"):
            print("- Loading voting results...")
            self.results = self.load_vote_results()
        else:
        ...
        print(f"---------- Voting for winners finished ----------\n")
    ...
    def save_vote_results(self):
        ...
        print(f"- Saved timestamp information for related tweets in timestamp_{self.filename}")

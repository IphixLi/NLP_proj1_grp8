...
def count_whole_word_occurrences(text, word):
    # Use regular expression to count whole word occurrences
...
def remove_parathesis(text: str) -> str:
    if text.find("(") == -1:
        return text
    return text[:text.find("(")].strip()

def remove_punct_space(s):
    return s.translate(str.maketrans('', '', string.punctuation + string.whitespace)).lower()

def calc_sim(str1, str2):
    s1 = remove_punct_space(str1)
    s2 = remove_punct_space(str2)
...
def about_human(award: str) -> bool:
    return "actor" in award or "actress" in award or "director" in award or "cecil" in award

def about_music(award: str) -> bool:
    return "song" in award or "score" in award
...
def remove_triple_letters(text: str) -> str:
    ...
    return remove_space(re.sub(pattern, '', text))

def remove_s(text: str) -> str:
    return text.replace("'s", "")

def keep_alphanum_and_hyphen(text: str) -> str:
    return re.sub(r'[^a-zA-Z0-9-. ]', '', text)

def remove_selected_words(text: str) -> str:
    pattern = r'\byay\w*|\byah\w*|\bohh\w*|\bcongrat\w*|\brt\b|\bshld\b|\bfinishing\b'
...
def name_cleaning(text: str) -> str:
    ...
    text = remove_s(text)
    text = keep_alphanum_and_hyphen(text)
